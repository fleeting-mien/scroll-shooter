MAX_X = 700  # временно
MAX_Y = 720  # размеры окна

FPS = 60

BORDER_X = 36
BORDER_Y = 28

SPAWN_SECONDS = 5  # количество секунд, за которое спавнится 3 вражеских корабля
SHOOTING_COEF = 10  # раз в сколько кадров будет вылетать пуля при зажатой мышке
BUFF_DURATION = 5  # количество секунд, которое длится бафф корабля игрока
DROP_CHANCE = 2  # раз в сколько смертей с врагов будет падать дроп
INTENSITY = 30  # enemy ships shoot one in INTENSITY ticks

BOSS_SCORE = 40  # при достижении какого счета появляется босс

DEFAULT_SPEED = 600 / FPS

ALLY_LIVES = 5  # начальное число жизней игрока
BOSS_LIVES = 100  # количество жизней босса

ENEMY_BULLET_DAMAGE = 1
LASER_DAMAGE = 0.15
ENEMY_SHIP_LIVES = 1
ASTEROID_LIVES = 5
